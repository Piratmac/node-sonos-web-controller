...
document.getElementById('play').addEventListener('click', function() {
    ...
    Socket.socket.emit('transport-state', {
        uuid: Sonos.currentState.selectedZone,
        state: 'play'
    });
});
...
document.getElementById('status-container').addEventListener('dblclick', function(e) {
    ...
    if (!li) return;
    Socket.socket.emit('seek', {
        uuid: Sonos.currentState.selectedZone,
        trackNo: li.dataset.trackNo
    });
});
